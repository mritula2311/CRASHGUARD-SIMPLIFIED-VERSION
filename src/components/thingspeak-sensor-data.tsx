...
export function ThingSpeakSensorData() {
  ...
  const fetchSensorData = async () => {
    try {
      ...
      // Check if the API response has a data property (wrapped response)
      const data: LiveSensorData = apiResponse.data || apiResponse;
      ...
      // Check if we have the expected sensor data structure
      if (!data.acceleration || !data.gyroscope || !data.vibration) {
        // Create a default structure if sensor data is missing
        const defaultData: LiveSensorData = {
          timestamp: data.timestamp || new Date().toISOString(),
          data_source: data.data_source || "demo_mode",
          acceleration: {
            x: 0,
            y: 0,
            z: 0,
          ...
          risk_assessment: {
            ...
            acceleration_level: "unknown",
            gyroscope_level: "unknown",
              ...
              <div className="space-y-3">
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">
                    {sensorData.acceleration?.magnitude?.toFixed(2) || "0.00"} m/s²
                  </div>
                  <div className="text-sm text-muted-foreground">Magnitude</div>
                </div>
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <div className="text-center">
                    <div className="font-medium">{sensorData.acceleration?.x?.toFixed(2) || "0.00"}</div>
                    <div className="text-muted-foreground">X-axis</div>
                  </div>
                  <div className="text-center">
                    <div className="font-medium">{sensorData.acceleration?.y?.toFixed(2) || "0.00"}</div>
                    <div className="text-muted-foreground">Y-axis</div>
                  </div>
                  <div className="text-center">
                    <div className="font-medium">{sensorData.acceleration?.z?.toFixed(2) || "0.00"}</div>
              ...
              <div className="space-y-3">
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-600">
                    {sensorData.gyroscope?.magnitude?.toFixed(2) || "0.00"} °/s
                  </div>
                  <div className="text-sm text-muted-foreground">Magnitude</div>
                </div>
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <div className="text-center">
                    <div className="font-medium">{sensorData.gyroscope?.x?.toFixed(2) || "0.00"}</div>
                    <div className="text-muted-foreground">X-axis</div>
                  </div>
                  <div className="text-center">
                    <div className="font-medium">{sensorData.gyroscope?.y?.toFixed(2) || "0.00"}</div>
                    <div className="text-muted-foreground">Y-axis</div>
                  </div>
                  <div className="text-center">
                    <div className="font-medium">{sensorData.gyroscope?.z?.toFixed(2) || "0.00"}</div>
                    <div className="text-muted-foreground">Z-axis</div>
                  </div>
                </div>
                {sensorData.risk_assessment && (
              ...
              <div className="space-y-3">
                <div className="text-center">
                  <div className="text-2xl font-bold text-orange-600">
                    {sensorData.vibration?.total_active || 0}/6
                  </div>
